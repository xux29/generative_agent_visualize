#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Launch 9 diabetes experiments in parallel - proper version"""
import os
import subprocess
import json
import time
...
for i, (init_health, discipline) in enumerate(EXPERIMENTS, 1):
    name = f'init{init_health}_{discipline}_20260506b'
    ...
    # Use shell=True on Windows with creation flags to hide window
    startupinfo = subprocess.STARTUPINFO()
    startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
    startupinfo.wShowWindow = subprocess.SW_HIDE

    p = subprocess.Popen(cmd, startupinfo=startupinfo)
    print(f'[{i}/9] {name} (PID={p.pid})')
    time.sleep(0.5)
