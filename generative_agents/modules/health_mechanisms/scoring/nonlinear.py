...
class NonlinearHealthScorer:
    ...
    def __init__(
        ...
        self.trough_count = 0
        self.early_sleep_streak = 0
        self.late_sleep_streak = 0
        self.violation_day_streak = 0
    ...
    def calculate_daily_change(
        ...
        from .timing import DEFAULTS, minute, sleep_change
        timing_config = {**DEFAULTS, **(get_path("simulation.health.timing", {}) or {})}
        timed = bool(agent_data.get("timing_observed"))
        weighted_count = actual_violations
        if timed:
            violations = [event for event in agent_data.get("violation_events", []) if not event.get("blocked")]
            actual_violations = len(violations)
            had_violation = actual_violations > 0
            unblocked_violation_count = actual_violations
            self.violation_day_streak = self.violation_day_streak + 1 if had_violation else 0
            weights = []
            for event in violations:
                t = minute(event.get("time"))
                weights.append(next((weight for end, weight in timing_config["violation_time_weights"]
                                     if t is not None and t < end), 1.0))
            multiplier = 1 + timing_config["violation_streak_speed"] * min(
                max(0, self.violation_day_streak-1), timing_config["violation_streak_cap"]-1)
            weighted_count = sum(weights) * multiplier
            breakdown["components"]["violation_timing"] = {"count": actual_violations,
                "weights": weights, "day_streak": self.violation_day_streak,
                "streak_multiplier": multiplier, "weighted_count": weighted_count}
        ...
        if had_violation:
            if intervention_success and unblocked_violation_count == 0:
                ...
                penalty = self._calculate_violation_penalty(
                    violation_count=weighted_count,
                    current_score=self.current_score
                )
                # Successful management still reduces exposure on a mixed day:
                # previously, one remaining violation discarded the benefit of
                # every blocked attempt.  Reuse the exposed management intensity
                # rather than introducing a hidden tuning parameter.
                if intervention_success and intervention_count > 0:
                    intensity = float(get_path("management.intervention.intensity", default=1.0))
                    mitigation = max(
                        0.15,
                        1.0 - min(3, intervention_count) * 0.30 * intensity,
                    )
                    raw_penalty = penalty
                    penalty *= mitigation
                    breakdown["components"]["intervention_mitigation"] = {
            ...
            # 2. 遵从奖励。初始 90 分且高自律者允许在长期良好表现下
            # 小幅超过基线；改善空间仍由已暴露的 complianceBonus 控制。
            # A compliant day also counts when no manager intervention was needed.
            if actual_violations == 0 and not timed:
                compliance_scale = float(get_path("simulation.health.compliance_bonus_scale", 1.0))
                improvement_headroom = (
                    min(4.0, compliance_scale * 2.0)
                    if self.initial_score >= 90 and self.original_discipline == SelfDisciplineLevel.HIGH
                    else 0.0
                )
                recovery_ceiling = min(100.0, self.initial_score + improvement_headroom)
                if self.current_score < recovery_ceiling:
        ...
        if timed:
            sleep_delta, self.early_sleep_streak, self.late_sleep_streak, detail = sleep_change(
                agent_data, timing_config, self.early_sleep_streak, self.late_sleep_streak)
            change += sleep_delta
            breakdown["components"]["sleep_timing"] = {**detail, "change": sleep_delta}
        ...
        # Most profiles recover only to baseline.  A 90/high profile may gain a
        # small complianceBonus-derived headroom, representing further health
        # optimisation rather than recovery from illness.
        if not had_violation and not timed:
            compliance_scale = float(get_path("simulation.health.compliance_bonus_scale", 1.0))
            improvement_headroom = (
                min(4.0, compliance_scale * 2.0)
                if self.initial_score >= 90 and self.original_discipline == SelfDisciplineLevel.HIGH
            ...
            new_score = min(float(self.initial_score) + improvement_headroom, new_score)
        ...
        # Persist the realised change after caps, so logs reconcile exactly.
    ...
    def _calculate_compliance_bonus(
        ...
        configured_bonus = get_path(
            f"simulation.health.discipline_params.{self.discipline_level.value}.compliance_bonus",
            default=1.5,
        )
        bonus = float(configured_bonus) * random.uniform(2.0 / 3.0, 4.0 / 3.0) * scale
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "early_sleep_streak": self.early_sleep_streak,
            "late_sleep_streak": self.late_sleep_streak,
            "violation_day_streak": self.violation_day_streak,
            # 【新增】自律提升机制
    ...
    def from_dict(cls, data: Dict) -> "NonlinearHealthScorer":
        ...
        scorer.early_sleep_streak = data.get("early_sleep_streak", 0)
        scorer.late_sleep_streak = data.get("late_sleep_streak", 0)
        scorer.violation_day_streak = data.get("violation_day_streak", 0)
        # 【新增】自律提升机制
...
"""
    ...
    def calculate_mood_score_with_discipline(
        ...
        """
        # The UI exposes ``simulation.satisfaction``.  This runtime path used
        # the legacy ``simulation.mood`` tree, so the expert sliders had no
        # effect on the score shown by the actual simulation.
        satisfaction_cfg = get_path("simulation.satisfaction", default=None) or {}
        BASE_SCORE = satisfaction_cfg.get("base_score", 6.0)
        ...
        freq_cfg = satisfaction_cfg.get("frequency_penalty", {})
        freq_per = freq_cfg.get("per_intervention", -0.5)
        freq_floor = freq_cfg.get("floor", freq_cfg.get("max_penalty", -3.0))
        ...
        # 2. 干预强度影响
        level_weights = satisfaction_cfg.get("intervention_level_weights", {})
        intensity_cfg = {
            "level_1": level_weights.get("1", level_weights.get(1, -0.3)),
            "level_2": level_weights.get("2", level_weights.get(2, -0.6)),
            "level_3": level_weights.get("3", level_weights.get(3, -1.0)),
        }
