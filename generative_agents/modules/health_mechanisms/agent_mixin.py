...
from modules.health_mechanisms.management.strategy import Strategy, StrategyManager
...
class HealthAgentMixin:
    ...
    def calculate_relapse_probability(self, is_relaxed=False, days_relaxed=0):
        ...
        tendency_add = self.relapse_tendency * tendency_weight

        # 综合计算
        relapse_prob = base_prob * discipline_mod * addiction_mod * time_mod * habit_mod + tendency_add
    ...
    def _get_kitchen_addresses(self):
        ...
        Returns:
            list: 厨房地址列表
        """
        from modules.health_mechanisms.scenario import get_agent_scenario
    ...
    def daily_reflection(self, day, day_log):
        ...
        # 日志主健康分使用0-100累计分，避免与legacy 0-10分混用。
        ...
        for target_name in target_scores.keys():
            ...
            score_breakdown = json.dumps(
                day_log.get("health_breakdown", target_data), ensure_ascii=False
            )
