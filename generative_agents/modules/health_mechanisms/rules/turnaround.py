    ...
    templates = {
        ...
        "kitchen": {
            "turnaround_monologue": (
                ...
                f"估计是{manager_name}把食物拿走了，只能先离开厨房。"
            ),
            "redirect_activity": "去客厅喝水休息",
            "redirect_location": "living_room",
...
def adjust_intervention_for_day(strategy, level2_used: bool, level3_used: bool):
    ...
    if not strategy:
        return strategy
    if strategy.level == 3 and not level2_used:
        strategy.level = 2
        strategy.action = "remove_food"
        strategy.reason = "首次物理干预先移除诱因；若之后仍反复尝试再升级锁定空间"
        return strategy
    if level3_used and strategy.level >= 2:
        strategy.level = 1
        strategy.action = "persuade"
        strategy.reason = f"{strategy.reason}（当天已执行过Level 3，降级为劝说）"
        return strategy
    if level2_used and strategy.level == 2:
        strategy.level = 1
        strategy.action = "persuade"
        strategy.reason = f"{strategy.reason}（当天已执行过Level 2，降级为劝说）"
        return strategy
    return strategy
