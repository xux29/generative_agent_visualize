...
class LongTermStrategyManager(StrategyManager):
    ...
    def _update_intervention_tracking(self, level: int, day: int):
        ...
        tracking = self._intervention_protection
        previous_day = tracking.get("last_tracking_day")
        if previous_day != day:
            tracking["previous_high_day_streak"] = (
                tracking["consecutive_high_intervention_days"] if previous_day == day - 1 else 0
            )
            tracking["last_tracking_day"] = day
            tracking["today_high_intervention"] = False
        tracking["today_high_intervention"] |= level >= min_high
        tracking["consecutive_high_intervention_days"] = (
            tracking["previous_high_day_streak"] + 1 if tracking["today_high_intervention"] else 0
        )
    ...
    def get_recommended_level(self, day: int, health_score: float = None) -> Tuple[int, str]:
        ...
        # 根据阶段调整 + 全局干预强度（management.intervention.intensity）
        phase_config = self.PHASE_CONFIG[self.current_phase]
        intervention_modifier = float(phase_config.get("intervention_modifier", 1.0))
        interv_cfg = _mgmt("intervention") or {}
        global_intensity = float(interv_cfg.get("intensity", 1.0))
        effective_tightness = intervention_modifier * global_intensity
        if effective_tightness < 0.75 and base_level > 0:
            base_level = max(0, base_level - 1)
            base_reason += f"（阶段/强度温和 {effective_tightness:.2f}）"
        elif effective_tightness > 1.25 and base_level < self.max_intervention_level:
            base_level = min(self.max_intervention_level, base_level + 1)
            base_reason += f"（阶段/强度偏严 {effective_tightness:.2f}）"
