...
路由：
- 只拧界面 22 个可调旋钮 → 用 set_ui_param（或 edit_config，路径须在白名单）
- 改公式/状态机/新机制 → 走机制 Skill（edit_code）；**不要**为拧旋钮去改 Python
- 涉及斯坦福基座、日循环编排、前端、密钥 → 拒绝

界面可调参数清单：docs/mechanism/界面可调参数.md（health / relapse / satisfaction / management 共 22 项）
...
每次回复用中文说明：改了什么、proposal/model id、是否已 validate、下一步。
"""
